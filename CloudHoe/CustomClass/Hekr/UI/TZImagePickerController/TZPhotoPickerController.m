...
- (void)viewDidLoad {
    ...
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithTitle: NSLocalizedString(@"取消", nil)  style:UIBarButtonItemStylePlain target:self action:@selector(cancel)];
...
- (void)configBottomToolBar {
    ...
    _previewButton.frame = CGRectMake(10, 3, 100, 44);
    ...
    if (tzImagePickerVc.allowPickingOriginalPhoto) {
        ...
        [_originalPhotoButton setTitle:NSLocalizedString(@"原图", nil) forState:UIControlStateNormal];
        [_originalPhotoButton setTitle:NSLocalizedString(@"原图", nil) forState:UIControlStateSelected];
    ...
    _okButton.frame = CGRectMake(self.view.tz_width - 100 - 12, 3, 100, 44);
    ...
    [_okButton setTitle:NSLocalizedString(@"确定", nil) forState:UIControlStateDisabled];
    ...
    _numberImageView.frame = isEN() == YES ? CGRectMake(self.view.tz_width - 70 - 24, 12, 26, 26) : CGRectMake(self.view.tz_width - 56 - 24, 12, 26, 26);
...
//        [alert show];
        ...
        [self showAlert:@"无法使用相机" msg:@"请在iPhone的""设置-隐私-相机""中允许访问相机" leftText:@"取消" leftCallback:nil rightText:@"设置" rigthCallback:^(UIAlertAction * _Nonnull action) {
            if (iOS8Later) {
