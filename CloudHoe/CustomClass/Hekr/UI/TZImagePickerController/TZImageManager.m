...
- (NSString *)getNewAlbumName:(NSString *)name {
    if (iOS8Later) {
        NSString *newName;
        if ([name rangeOfString:@"Roll"].location != NSNotFound)         newName = NSLocalizedString(@"相机胶卷", nil);
        else if ([name rangeOfString:@"Stream"].location != NSNotFound)  newName = NSLocalizedString(@"我的照片流", nil);
        else if ([name rangeOfString:@"Added"].location != NSNotFound)   newName = NSLocalizedString(@"最近添加", nil);
        else if ([name rangeOfString:@"Selfies"].location != NSNotFound) newName = NSLocalizedString(@"自拍", nil);
        else if ([name rangeOfString:@"shots"].location != NSNotFound)   newName = NSLocalizedString(@"截屏", nil);
        else if ([name rangeOfString:@"Videos"].location != NSNotFound)  newName = NSLocalizedString(@"视频", nil);
        else if ([name rangeOfString:@"Panoramas"].location != NSNotFound)  newName = NSLocalizedString(@"全景照片", nil);
        else if ([name rangeOfString:@"Favorites"].location != NSNotFound)  newName = NSLocalizedString(@"个人收藏", nil);
        else newName = name;
        return newName;
    } else {
        return name;
